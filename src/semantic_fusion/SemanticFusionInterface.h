...
public:
    ...
    , colour_threshold_(colour_threshold)
  {
    ...
    class_probabilities_gpu_.reset(new torch::Tensor(torch::empty({1,1,num_classes_,max_components_}, torch::ScalarType::Float)));
    class_probabilities_gpu_buffer_.reset(new torch::Tensor(torch::empty({1,1,num_classes_,max_components_}, torch::ScalarType::Float)));
    // This contains two rows - one is the max class (if none then negative) the
    // other is the probability
    class_max_gpu_.reset(new torch::Tensor(torch::empty({1,1,3,max_components_}, torch::ScalarType::Float)));
    class_max_gpu_buffer_.reset(new torch::Tensor(torch::empty({1,1,3,max_components_}, torch::ScalarType::Float)));
    rendered_class_probabilities_gpu_.reset(new torch::Tensor(torch::empty({1,num_classes_,640,480}, torch::ScalarType::Float)));
  }
  virtual ~SemanticFusionInterface() {}

  int UpdateSurfelProbabilities(const int surfel_id, const std::vector<float>& class_probs);
  void UpdateProbabilities(std::shared_ptr<torch::Tensor> probs,const std::unique_ptr<ElasticFusionInterface>& map);
  ...
  std::shared_ptr<torch::Tensor> get_rendered_probability();
  std::shared_ptr<torch::Tensor> get_class_max_gpu();
...
private:
  ...
  std::shared_ptr<torch::Tensor> class_probabilities_gpu_;
  int current_table_size_;
  // This is used to store the table swap after updating
  std::shared_ptr<torch::Tensor> class_probabilities_gpu_buffer_;
  std::shared_ptr<torch::Tensor> class_max_gpu_;
  std::shared_ptr<torch::Tensor> class_max_gpu_buffer_;
  // This stores the rendered probabilities of surfels from the map
  std::shared_ptr<torch::Tensor> rendered_class_probabilities_gpu_;
